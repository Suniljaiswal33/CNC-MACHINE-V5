...
# =========================
# ✅ HEX SETTINGS (SIDE BASED - FINAL)
# =========================
HEX_SIDE = 50

HEX_RADIUS = HEX_SIDE              # correct for RegularPolygon
dx = 4 * HEX_SIDE                  # horizontal spacing
dy = math.sqrt(5) * HEX_SIDE       # vertical spacing

SAFE_OFFSET = THICKNESS + HEX_SIDE + 5   # wall safety
...
with BuildPart() as part:
    ...
    # SHELL
    # =========================
    outer_wire = Wire.combine(base_face.outer_wire().edges())[0]
    ...
    with BuildSketch(top_face):
        ...
        with GridLocations(dx, dy, nx, ny):
            RegularPolygon(radius=HEX_RADIUS, side_count=6, rotation=60)

        # staggered grid
        with Locations((dx/2, dy/2)):
            with GridLocations(dx, dy, nx, ny):
                RegularPolygon(radius=HEX_RADIUS, side_count=6, rotation=30)
...
print("✅ FINAL PERFECT HONEYCOMB CREATED:", OUTPUT_FILE)
